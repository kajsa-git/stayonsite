...
- **Auto-generation:** `scripts/generate-article.mjs` calls Codex API with web search
...
- `public/llms.txt` — AI-readable company info for LLM crawlers (ChatGPT, Perplexity, Codex)
...
### CRM-statusregel for Codex

- Nar Kajsa ber Codex lasa eller andra nagot i CRM ska Codex alltid redovisa vilka poster som berors och foresla konkreta status-/uppfoljningsandringar for dem.
- Efter att Codex har hjalpt till att svara en uthyrare som registrerat sin bostad ska Codex alltid fraga Kajsa om den oppna uthyrarkontakten ska klarmarkeras (`bekraftad`/avslutad), ligga kvar i dialog eller fa ett nytt uppfoljningsdatum.
- Samma regel galler befintliga SMS-tradar: om Codex ser att Kajsa har svarat en CRM-kontakt ska Codex lyfta den berorda uppfoljningen och fraga om den ska stangas, ligga kvar i dialog eller planeras om.
- Codex far inte tyst klarmarkera eller ta bort poster ur en CRM-ko. Be om Kajsas klartecken innan statusen andras, om hon inte redan uttryckligen har bett om just den statusandringen i samma uppdrag.
