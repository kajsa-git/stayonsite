...
// Företag → kontakter, förfrågningar (+ deras matchningar), noter, mejl.
// ownerOutreach.requestId nollställs (rundan tillhör objektet, inte företaget).
...
// Förfrågan → dess matchningar. ownerOutreach.requestId nollställs.
...
// Objekt → matchningar, uthyrar-rundor, kontaktlogg, bildrader. requests.wonPropertyId
// nollställs (det är en lös referens, inte en FK). R2-objekten städas separat i
// bild-routern innan raderingen (se properties/[id]/route.ts).
