...
function propertyRow(p: Property, owner: Owner | null): SearchIndexInsert {
  ...
  return {
    ...
    subtitle: [[p.postalCode, p.city].filter(Boolean).join(" "), owner?.name].filter(Boolean).join(" · ") || null,
    keywords: kw(
      "objekt bostad",
      p.address,
      p.postalCode,
      ...
      p.specialNote,
      p.availability,
      p.bedrooms != null ? `${p.bedrooms} sovrum` : null,
      p.squareMeters != null ? `${p.squareMeters} kvm` : null,
      label,
    ),
