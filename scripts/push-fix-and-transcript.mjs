#!/usr/bin/env node
/**
 * 1. Deletes old insikter page (broken åäö)
 * 2. Creates new insikter page with correct Swedish chars
 * 3. Creates transcription page
 * Both under "Insikter, Mats" parent
 */
...
// Parent page ID: "Insikter, Mats"
const PARENT_ID = "301e0171c36c81e88769f0eb320be367";
// Old broken insikter page to archive
const OLD_INSIKTER_ID = "301e0171c36c818dabbecce8d51c5d1c";
...
async function main() {
  // 1. Archive old broken insikter page
  console.log("Archiving old insikter page (broken åäö)...");
  try {
    await notionRequest(`/pages/${OLD_INSIKTER_ID}`, { archived: true }, "PATCH");
    console.log("  Old page archived ✓\n");
  } catch (e) {
    console.log("  Could not archive old page (may already be archived)\n");
  }

  // 2. Create new insikter page with correct Swedish
  console.log("Creating new insikter page with correct åäö...");
  const insikterMd = readFileSync(resolve(ROOT, "docs/mats-insikter-2026-02-08.md"), "utf-8");
  await createPage(PARENT_ID, "Kundinsikter: BS Logistics / Mats Johansson", "\uD83D\uDCDD", insikterMd);

  // 3. Create transcription page
  console.log("\nCreating transcription page...");
  const transcriptMd = readFileSync(resolve(ROOT, "docs/transkription-mats-samtal-2026-02-08.md"), "utf-8");
  await createPage(PARENT_ID, "Transkription: Samtal David & Kajsa", "\uD83C\uDFA4", transcriptMd);
