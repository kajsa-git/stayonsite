...
export async function generateMetadata({ params }: { params: Promise<{ citySlug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `Zakwaterowanie pracownicze i mieszkania firmowe ${city.name} | StayOnSite`,
    description: truncateDescription(getLocalizedText(city.intro, 'pl')),
    keywords: getLocalizedKeywords(city.keywords, 'pl').join(', ') + ', StayOnSite',
    canonical: pl,
    hreflangs: [
      { lang: 'sv', href: sv },
      { lang: 'en', href: en },
...
export default async function Page({ params }: { params: Promise<{ citySlug: string }> }) {
  ...
  const city = cities.find((c) => c.slug === citySlug)
  if (!city) notFound()
  return <CityPage citySlug={citySlug} locale="pl" city={city} nearbyCities={getNearbyCities(citySlug)} />
}
